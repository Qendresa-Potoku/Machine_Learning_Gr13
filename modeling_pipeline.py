...
def predict_pipeline(
    ...
    final_predictions = np.empty(len(X_test), dtype=float)
    normal_mask = predicted_class == 0
    extreme_mask = predicted_class == 1

    if normal_mask.any():
        final_predictions[normal_mask] = model_normal.predict(X_test.loc[normal_mask])
    if extreme_mask.any():
        final_predictions[extreme_mask] = model_extreme.predict(X_test.loc[extreme_mask])
...
def train_dual_model(split_data: dict) -> dict:
    ...
    final_predictions, predicted_class = predict_pipeline(classifier, model_normal, model_extreme, X_test)
